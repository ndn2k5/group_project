...
class Database:
    ...
    def get_user_by_email(self, email):
        conn = self.get_connection()
        c = conn.cursor()
        c.execute('SELECT id, email, password, name, avatar, theme FROM users WHERE email = ?', (email,))
        user = c.fetchone()
        conn.close()
        if user:
            return {
    ...
    def get_user_by_id(self, user_id):
        conn = self.get_connection()
        c = conn.cursor()
        c.execute('SELECT id, email, name, avatar, theme FROM users WHERE id = ?', (user_id,))
        user = c.fetchone()
        conn.close()
        if user:
            return {
